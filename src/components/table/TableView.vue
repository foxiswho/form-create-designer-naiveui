                            ...
                            <div class="_fd-table-view-cell">
                                <DragTool :drag-btn="false" :handle-btn="true" @active="active({pid, idx})"
                                    ...
                                    <template #handle>
                                        ...
                                        <div class="_fd-drag-btn _fd-table-view-btn">
                                            <n-dropdown trigger="click" :options="options(pid, idx,lattice,rule)">
...
export default defineComponent({
    ...
    methods: {
        ...
        options(pid, idx,lattice,rule){
          return [
            {
              label: t('tableOptions.addLeft'),
              key: 'addCol_1',
              props: {
                onClick: () => {
                  this.command(['addCol', [{pid,idx,data: lattice[pid][idx]}, 1]])
                }
              }
            },
            {
              label: t('tableOptions.addRight'),
              key: 'addCol_0',
              props: {
                onClick: () => {
                  this.command(['addCol', [{pid,idx,data: lattice[pid][idx]}, 0]])
                }
              }
            },
            {
              label: t('tableOptions.addTop'),
              key: 'addRow_1',
              props: {
                onClick: () => {
                  this.command(['addRow', [{pid,idx,data: lattice[pid][idx]}, 1]])
                }
              }
            },
            {
              label: t('tableOptions.addBottom'),
              key: 'addRow_0',
              props: {
                onClick: () => {
                  this.command(['addRow', [{pid,idx,data: lattice[pid][idx]}, 0]])
                }
              }
            },
            {
              label: t('tableOptions.mergeRight'),
              key: 'mergeRight',
              props: {
                onClick: () => {
                  this.command(['mergeRight', [{pid,idx,data: lattice[pid][idx]}]])
                }
              },
              disabled: lattice[pid][idx].right,
            },
            {
              label: t('tableOptions.mergeBottom'),
              key: 'mergeBottom',
              props: {
                onClick: () => {
                  this.command(['mergeBottom', [{pid,idx,data: lattice[pid][idx]}]])
                }
              },
              disabled: lattice[pid][idx].bottom,
            },
            {
              label: t('tableOptions.splitCol'),
              key: 'splitCol',
              props: {
                onClick: () => {
                  this.command(['splitCol', [{pid,idx,data: lattice[pid][idx]}]])
                }
              },
              disabled: !(lattice[pid][idx].layout && lattice[pid][idx].layout.col > 1),
            },
            {
              label: t('tableOptions.splitRow'),
              key: 'splitRow',
              props: {
                onClick: () => {
                  this.command(['splitRow', [{pid,idx,data: lattice[pid][idx]}]])
                }
              },
              disabled: !(lattice[pid][idx].layout && lattice[pid][idx].layout.row > 1),
            },
            {
              label: t('tableOptions.rmCol'),
              key: 'rmCol',
              props: {
                onClick: () => {
                  this.command(['rmCol', [{pid,idx,data: lattice[pid][idx]}]])
                }
              },
              disabled: rule.col < 2,
            },
            {
              label: t('tableOptions.rmRow'),
              key: 'rmRow',
              props: {
                onClick: () => {
                  this.command(['rmRow', [{pid,idx,data: lattice[pid][idx]}]])
                }
              },
              disabled: rule.col < 2,
            },
